...
const ConsignMoneyModal = ({ show, handleClose, handleConsign }) => {
    const [amount, setAmount] = useState(0);
    const [accessKey, setAccessKey] = useState("");

    const handleSubmit = (e) => {
        e.preventDefault();
        handleConsign({ amount, accessKey });
        setAmount("");
        setAccessKey("");
    };
    ...
    return (
        <Modal show={show} onHide={handleClose} centered className="custom-modal">
            <Modal.Header closeButton className="bg-light">
                <Modal.Title style={{ color: '#8671ff' }}>Consignar Dinero</Modal.Title>
            </Modal.Header>
            <Modal.Body className="p-4">
                <Form onSubmit={handleSubmit}>
                    <Form.Group controlId="formAmount" className="mb-3">
                        <Form.Label>Monto a Consignar</Form.Label>
                        <Form.Control
                            type="number"
                            value={amount}
...
export default ConsignMoneyModal;
